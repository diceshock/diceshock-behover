...
export const Route = createFileRoute("/{-$storeLocale}/_with-home-lo/riichi")({
  head: () => ({
    meta: [
      { title: "日麻 PP 排行榜 - DiceShock 骰子奇兵" },
      {
        name: "description",
        content: "DiceShock 骰子奇兵立直麻将 PP 排行榜，查看日/周/月排名",
      },
      { property: "og:title", content: "日麻 PP 排行榜 - DiceShock 骰子奇兵" },
      {
  ...
  return (
    ...
    >
      ...
      <p className="font-bold text-sm sm:text-base truncate">
        {entry.nickname}
        {isMe && (
          <span className="text-primary ml-1 text-xs">{t("riichi.me")}</span>
        )}
      </p>
      <p
        className={clsx(
