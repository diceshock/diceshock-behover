...
export const Route = createFileRoute("/dash/$store/events_/$id")({
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        msg.success("活动已保存");
        navigate({ to: "/dash/$store/events", params: { store } });
      } catch (err) {
    ...
    <ClientOnly>
      <main className="size-full overflow-y-auto">
        <div className="px-4 pt-4">
          <DashBackButton to="/dash/$store/events" params={{ store }} />
        </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="flex justify-end gap-3 mt-4">
              <button
                ...
                onClick={() => navigate({ to: "/dash/$store/events", params: { store } })}
                disabled={submitting}
              >
                取消
              </button>
