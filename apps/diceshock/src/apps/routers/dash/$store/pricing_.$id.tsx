...
export const Route = createFileRoute("/dash/$store/pricing_/$id")({
...
function PricingDetailPage() {
  ...
  const handleSave = () => {
    ...
    void navigate({ to: "/dash/$store/pricing", params: { store } });
  };
    ...
    return (
      <main className="size-full flex flex-col items-center justify-center gap-4">
        ...
        <button
          ...
          onClick={() => navigate({ to: "/dash/$store/pricing", params: { store } })}
        >
          返回价格计划
        </button>
      </main>
    ...
    <ClientOnly>
      <main className="size-full overflow-y-auto">
        <div className="px-4 pt-4">
          <DashBackButton to="/dash/$store/pricing" params={{ store }} />
        </div>
