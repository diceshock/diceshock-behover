...
vi.mock("@/server/apis/wechat/messagePipeline", () => ({
  sendStatusMessage: mocks.sendStatusMessage,
}));

vi.mock("@/server/apis/wechat/tools/query", async (importOriginal) => ({
  ...(await importOriginal<typeof import("@/server/apis/wechat/tools/query")>()),
  executeQueryTool: mocks.executeQueryTool,
}));

vi.mock("@/server/apis/wechat/tools/mutate", () => ({
  executeMutateTool: mocks.executeMutateTool,
}));

vi.mock("@/server/apis/wechat/tools/loadSkill", async (importOriginal) => ({
  ...(await importOriginal<Record<string, unknown>>()),
  executeLoadSkillTool: mocks.executeLoadSkillTool,
}));

import { chatWithAgent } from "@/server/apis/wechat/deepseekClient";
